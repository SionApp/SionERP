...
export const DiscipleshipAnalyticsSection = ({ discipleshipStats }: DiscipleshipAnalyticsSectionProps) => {
  const { 
    analytics, 
    zoneStats, 
    groupPerformance, 
          ...
          <CardContent className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              {alerts.slice(0, 3).map((alert) => (
                <div key={alert.id} className="flex items-start space-x-3 p-3 rounded-lg bg-accent/20 border border-accent/30">
            ...
            <div className="space-y-2">
              <h4 className="font-semibold text-sm text-muted-foreground">Multiplicaciones Recientes</h4>
              {multiplications.slice(0, 3).map((mult, index) => (
                <div key={index} className="flex items-center justify-between p-3 rounded-lg bg-green-500/10 border border-green-500/20">
                  ...
                  <Badge 
                    ...
                    className="text-xs"
                  >
                    {mult.status === 'successful' ? '✅ Exitosa' : 
                     mult.status === 'planned' ? '📅 Planeada' : '⚠️ En proceso'}
                  </Badge>
                </div>
              ))}
            </div>
        ...
        <CardContent>
          <div className="space-y-3">
            {groupPerformance.slice(0, 6).map((leader, index) => (
              <div key={leader.groupId} className="flex items-center space-x-4 p-4 rounded-xl bg-gradient-to-r from-accent/30 to-transparent border border-border/50">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center space-x-2">
                    ...
                    <Badge variant="outline" className="text-xs">{leader.groupName}</Badge>
                  </div>
                  <div className="flex items-center space-x-4 mt-1">
                    <div className="flex items-center space-x-1">
                      <Users className="h-3 w-3 text-muted-foreground" />
                      <span className="text-xs text-muted-foreground">{leader.attendance}% asistencia</span>
                    </div>
                    <div className="flex items-center space-x-1">
                      <Heart className="h-3 w-3 text-muted-foreground" />
                      <span className="text-xs text-muted-foreground">{leader.spiritualHealth}/10 salud</span>
                    </div>
                    <div className="flex items-center space-x-1">
                      <TrendingUp className="h-3 w-3 text-muted-foreground" />
                      <span className={`text-xs ${getTrendColor(leader.growth)}`}>
                        {leader.growth > 0 ? '+' : ''}{leader.growth}% crecimiento
                      </span>
                    </div>
                  </div>
                </div>
