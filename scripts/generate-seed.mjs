...
// PII real removida de acá (era código, no dato de config — no debería haber
// vivido en git nunca). Si hace falta regenerar el seed real desde el Excel,
// completar este array localmente desde una fuente NO comiteada (ver Engram,
// proyecto "sionerp", topic security/seed-sql-pii-exposure) y no volver a
// commitear el archivo con los valores puestos.
const COORDINADORES = [];
